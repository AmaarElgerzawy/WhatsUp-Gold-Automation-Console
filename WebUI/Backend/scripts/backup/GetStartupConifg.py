...
for ip in ips:
    ...
    try:
        ...
        print(f"Getting start-config from {ip} ...")
        startup_config = conn.send_command("show startup-config", expect_string=r"#")

        conn.disconnect()

        # Save to file
        timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        filename = Path(OUTPUT_DIR/ ip / f"startup_config_{timestamp}.txt")
        filename.parent.mkdir(parents=True, exist_ok=True)
        filename.write_text(startup_config, encoding="utf-8")
