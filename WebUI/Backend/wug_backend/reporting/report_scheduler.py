...
from wug_backend.reporting.availability_report import AvailabilityReportService, OUTPUT_FOLDER as AVAILABILITY_OUTPUT_FOLDER
...
class ReportScheduler:
    def __init__(self) -> None:
        self._availability = AvailabilityReportService()
        self._uptime = DeviceUpTimeReportService()

    def send_all_reports_via_email(self, start_date, end_date):
        ...
        for report_path in report_files:
            try:
                ...
                attachments.append({"content": list(content_bytes), "filename": os.path.basename(report_path)})
            except Exception as e:
                print(f"[EMAIL] Failed to read attachment {report_path}: {e}")

        params = {"from": RESEND_FROM, "to": [RESEND_TO], "subject": subject, "html": html_body}
        if attachments:
    ...
    def send_single_report_email(self, group_name, report_path, start_date, end_date):
    ...
    def _trigger_fires(self, job: dict, now: datetime, last_run_iso: str):
        ...
        if not run_day:
            return False, None
        wd = self._weekday_index(str(run_day))
        if now.weekday() != wd:
            return False, None
        if now < anchor:
            return False, anchor
    ...
    def _compute_window_from_trigger(self, anchor_dt: datetime, job: dict):
        schedule_type = (job.get("type") or "weekly").strip().lower()
        ...
        if schedule_type == "monthly":
            ...
            sh, sm = self._parse_time(job.get("period_start_time") or "00:00")
            eh, em = self._parse_time(job.get("period_end_time") or "23:59")
        ...
        start_wd = self._weekday_index(str(job.get("period_start_day") or "mon"))
        end_wd = self._weekday_index(str(job.get("period_end_day") or "sun"))
    ...
    def run_scheduled_reports(self):
        jobs = self.load_schedule_config()
        ...
        if isinstance(state, dict):
            keys_to_delete = [
                k
                for k in state
                if "::__" in k and k != "__MONTHLY_ALL__" and k.split("::__", 1)[0] not in valid_job_ids
            ]
        ...
        if do_monthly:
            ...
            start_m, end_m = self.get_previous_month_range()
            groups = self._availability.get_device_groups()
            for gid, gname in groups:
                print(f"[MONTHLY RUN] {gname}: {start_m} -> {end_m}")
                try:
                    self._availability.write_excel_for_group(
                        device_group_id=gid, start_date=start_m, end_date=end_m, group_name=gname
                    )
                except Exception as e:
                    print(f"[MONTHLY ERROR] {gname}: {e}")
            self.send_all_reports_via_email(start_m, end_m)
            state["__MONTHLY_ALL__"] = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0).isoformat()

        name_to_id = {name: gid for gid, name in self._availability.get_device_groups()}
        ...
        for idx, job in enumerate(jobs):
            ...
            should_run, anchor_dt = self._trigger_fires(job, now, last_run_iso)
            if not should_run or anchor_dt is None:
                continue

            try:
                start_date, end_date = self._compute_window_from_trigger(anchor_dt, job)
            except Exception as e:
            ...
            if run_reports in ("availability", "both"):
                try:
                    ...
                    availability_report_path = self._availability.write_excel_for_group(
                        device_group_id=device_group_id, start_date=start_date, end_date=end_date, group_name=group_name_stripped
                    )
                    if availability_report_path:
                        self.send_single_report_email(group_name_stripped, availability_report_path, start_date, end_date)
                except Exception as e:
            ...
            if run_reports in ("uptime", "both"):
                try:
                    ...
                    uptime_rows = self._uptime.run_sp_group_device_uptime(device_group_id, start_date, end_date)
                    if uptime_rows:
                        uptime_report_path = self._uptime.write_excel(group_name_stripped, uptime_rows, start_date, end_date)
                        self.send_single_report_email(group_name_stripped, uptime_report_path, start_date, end_date)
                    else:
...
def run_scheduled_reports():
    ReportScheduler().run_scheduled_reports()
