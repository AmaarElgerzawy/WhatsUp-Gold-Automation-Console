"""
OOP packaging of the former scripts/reporting/AvailabilityReport.py logic.

CRITICAL: Logic parity is maintained by porting code verbatim into methods.
"""
...
class AvailabilityReportService:
    def get_duration_from_seconds(self, total_seconds: int) -> str:
    ...
    def get_active_monitor_availability(self, device_group_id: int, start_date: datetime, end_date: datetime):
...
"""
        ...
        for row in rows:
            ...
            up_dur = self.get_duration_from_seconds(rec["nUpSeconds"])
            down_dur = self.get_duration_from_seconds(rec["nDownSeconds"])
            maint_dur = self.get_duration_from_seconds(rec["nMaintenanceSeconds"])
            unknown_dur = self.get_duration_from_seconds(rec["nUnknownSeconds"])
            total_dur = self.get_duration_from_seconds(rec["nTotalSeconds"])
    ...
    def write_excel_for_group(self, device_group_id: int, start_date: datetime, end_date: datetime, group_name: str | None = None):
        rows = self.get_active_monitor_availability(device_group_id, start_date, end_date)
        if not rows:
