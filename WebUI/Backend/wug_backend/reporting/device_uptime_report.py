...
class DeviceUpTimeReportService:
    ...
    def get_device_extra_data(self, device_ids):
    ...
    """
        ...
        for row in cur.fetchall():
            data[row.nDeviceID] = {"Note": row.sNote}
    ...
    def run_sp_group_device_uptime(self, device_group_id: int, start_date: datetime, end_date: datetime):
...
"""
        ...
        for row in rows:
            ...
            results.append(
                {
                    ...
                    "UpDuration": self.get_duration_from_seconds(rec["nUpSeconds"]),
                    "Maintenance": pct(rec["nMaintenancePercent"]),
                    "MaintenanceDuration": self.get_duration_from_seconds(rec["nMaintenanceSeconds"]),
                    "Unknown": pct(rec["nUnknownPercent"]),
                    "UnknownDuration": self.get_duration_from_seconds(rec["nUnknownSeconds"]),
                    "Down": pct(rec["nDownPercent"]),
                    "DownDuration": self.get_duration_from_seconds(rec["nDownSeconds"]),
                    "TotalDuration": self.get_duration_from_seconds(rec["nTotalSeconds"]),
                }
            )
            ...
            extra_data = self.get_device_extra_data(device_ids)
    ...
    def write_excel(self, group_name, rows, start_date, end_date):
        os.makedirs(OUTPUT_FOLDER, exist_ok=True)

        safe_name = "".join(c if c.isalnum() or c in "-_" else "_" for c in group_name)
        path = os.path.join(OUTPUT_FOLDER, f"DeviceUpTime_{safe_name}.xlsx")
